...
const EmployeeTable = ({ loading, error, searchTerm, setSearchTerm, employees, isDeleteMode, onDeleteClick }) => {
  return (
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
                ...
                <tr key={idx} className="animate-pulse">
                  ...
                  {isDeleteMode && (
                    <td className="px-6 py-4 text-right">
                      <div className="h-8 bg-slate-200 rounded-xl w-8 ml-auto"></div>
                    </td>
                  )}
                </tr>
              ))
              ...
              <tr>
                <td colSpan={isDeleteMode ? 6 : 5} className="px-6 py-8 text-center text-red-500 font-semibold">
                  {error}
                </td>
              </tr>
            ) : employees.length === 0 ? (
              <tr>
                <td colSpan={isDeleteMode ? 6 : 5} className="px-6 py-8 text-center text-slate-400 font-semibold">
                ...
                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-bold ${
                      emp.employee_status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                    }`}>
