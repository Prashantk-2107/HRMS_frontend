    ...
    <div className="flex flex-col gap-6 text-left min-h-screen pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-500">Access corporate policies, contracts, and manage personal identity documents.</p>
        </div>
                ...
                {DOCUMENT_TYPES.map((type) => {
                  ...
                  return (
                    <motion.div
                      key={type.key}
                      whileHover={{ y: -3 }}
                      className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between h-[210px] hover:shadow-md hover:border-slate-200/80 transition-all duration-200"
                    >
                      <div>
                        <div className="flex justify-between items-start gap-2 mb-2">
                          <h3 className="font-bold text-slate-800 text-base">{type.label}</h3>
                          {userDoc ? (
                            <span
                            ...
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full border bg-slate-50 text-slate-400 border-slate-100">
                              Missing
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-slate-400 font-medium mb-3 line-clamp-2">{type.description}</p>

                        {userDoc && (
                          <div className="text-xs text-slate-500 font-medium space-y-1">
                            {userDoc.document_number && (
                              <p>
                                <span className="text-slate-400 font-bold uppercase text-[9px] tracking-wider block">Doc Number</span>
                                <span className="text-slate-700 font-semibold">{userDoc.document_number}</span>
                              </p>
                            )}
                            {userDoc.verification_status === 'rejected' && userDoc.rejection_reason && (
                              <div className="mt-1 flex items-start gap-1 p-2 rounded-xl bg-rose-50/50 border border-rose-100/50">
                        ...
                        {userDoc ? (
                          <>
                            <button
                              ...
                              className="flex items-center gap-1 hover:bg-slate-50 text-slate-600 border border-slate-200 px-3 py-1.5 rounded-xl text-xs font-semibold cursor-pointer active:scale-95 transition-all"
                            >
                              <Eye size={12} />
                              <span>View</span>
                            </button>
                            ...
                            {userDoc.verification_status === 'rejected' && (
                              <button
                                ...
                                className="flex items-center gap-1.5 bg-rose-50 border border-rose-200/80 text-rose-600 hover:bg-rose-100 hover:text-rose-700 px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer active:scale-95 transition-all"
                              >
                                <Upload size={12} />
                                <span>Re-upload</span>
                              </button>
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 bg-slate-50 border border-slate-200/80 text-indigo-600 hover:bg-indigo-50/40 hover:border-indigo-100 hover:text-indigo-700 px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer active:scale-95 transition-all"
                          >
                            <Upload size={12} />
          ...
          >
            ...
            <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-2xl border border-slate-200 text-sm font-medium focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-slate-800 transition-all placeholder:text-slate-400 bg-slate-50/30"
                />
              </div>

              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-1.5 bg-slate-50/50 border border-slate-200 px-3 py-2 rounded-2xl">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-3 py-2 rounded-2xl cursor-pointer active:scale-95 text-slate-600 text-xs font-bold transition-all"
                >
                  <ArrowUpDown size={14} />
            ...
            {allDocsLoading ? (
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-12 text-center flex flex-col items-center justify-center">
                <Folder className="text-slate-300 mb-4" size={48} />
                <h3 className="font-bold text-slate-800 mb-1">No documents found</h3>
                <p className="text-sm text-slate-400 max-w-sm">No employee documents match your current filter parameters or search terms.</p>
              </div>
            ) : (
                          ...
                          <tr key={doc.document_id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="py-4 px-6">
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-indigo-50 border border-indigo-100/50 flex items-center justify-center text-indigo-600 font-bold text-xs select-none">
                            ...
                            <td className="py-4 px-6">
                              <div className="flex flex-col items-start gap-1">
                                ...
                                {doc.verification_status === 'rejected' && doc.rejection_reason && (
                                  <span className="text-[10px] text-rose-600 font-semibold max-w-[200px] truncate" title={doc.rejection_reason}>
        ...
        {isUploadOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <motion.div
              ...
              className="bg-white rounded-3xl border border-slate-100 shadow-xl max-w-lg w-full overflow-hidden"
            >
              <div className="flex justify-between items-center px-6 py-5 border-b border-slate-50">
                <h3 className="font-bold text-slate-800 text-base">Upload Document</h3>
                <button
              ...
              <form onSubmit={handleUploadSubmit} className="p-6 space-y-4">
                ...
                {canUpload ? (
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      required
                      className="w-full bg-slate-50 border border-slate-200/80 rounded-xl px-3 py-2.5 outline-none focus:ring-1 focus:ring-indigo-500 text-sm font-semibold text-slate-700 transition-all"
                    >
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Employee</label>
                    <div className="w-full bg-slate-100 border border-slate-200/60 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-500">
                ...
                {/* Document Number */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Document Number / ID</label>
                  <input
                    type="text"
                    placeholder="Enter document number (optional)"
                    value={uploadDocNumber}
                    onChange={(e) => setUploadDocNumber(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200/80 rounded-xl px-3 py-2.5 outline-none focus:ring-1 focus:ring-indigo-500 text-sm font-semibold text-slate-700 transition-all placeholder:text-slate-400"
                  />
                </div>

                {/* Custom Document Name */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Document Display Name</label>
                  <input
                    type="text"
                ...
                {/* File Upload Area */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Attach File *</label>
                  <div
                    ...
                    <label className="flex flex-col items-center gap-1.5 w-full h-full cursor-pointer">
                      ...
                      {uploadFile ? (
                        <div className="space-y-1">
                          <span className="text-xs font-semibold text-slate-700 block max-w-[250px] truncate">
                            {uploadFile.name}
                          </span>
                          <span className="text-[10px] text-slate-400 block font-semibold">
                        ...
                        <>
                          <span className="text-xs font-bold text-slate-600">Drag & drop files or click here</span>
                          <span className="text-[10px] text-slate-400 font-bold uppercase">Max limit: 5MB</span>
                        </>
                      )}
                      <input
                        type="file"
                ...
                <div className="flex justify-end gap-3 pt-3 border-t border-slate-50">
                  <button
                    type="button"
                    onClick={closeUploadModal}
                    className="px-4 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold text-sm transition-all cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
      ...
      <AnimatePresence>
        {isVerifyOpen && selectedDocToVerify && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            ...
            >
              <div className="flex justify-between items-center px-6 py-5 border-b border-slate-50">
                <h3 className="font-bold text-slate-800 text-base">Verify Document</h3>
                <button
                  onClick={() => {
                    setIsVerifyOpen(false);
                    setSelectedDocToVerify(null);
              ...
              <form onSubmit={handleVerifySubmit} className="p-6 space-y-4">
                <div className="bg-slate-50 p-4 rounded-2xl text-xs space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-400 font-bold">Doc Type</span>
                  ...
                  {selectedDocToVerify.document_number && (
                    <div className="flex justify-between">
                      <span className="text-slate-400 font-bold">Doc ID/Number</span>
                      <span className="text-slate-700 font-bold">
                        {selectedDocToVerify.document_number}
                      </span>
                    </div>
                  ...
                  <div className="flex justify-between items-center border-t border-slate-200/50 pt-2 mt-2">
                    <span className="text-slate-400 font-bold">File Link</span>
                    <button
                      type="button"
                ...
                {/* Status Switcher */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Action Status *</label>
                ...
                {verifyStatus === 'rejected' && (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Rejection Reason *</label>
                    <textarea
                      required
                      rows={3}
                      ...
                      className="w-full bg-slate-50 border border-slate-200/80 rounded-xl px-3 py-2 outline-none focus:ring-1 focus:ring-indigo-500 text-sm font-semibold text-slate-700 transition-all resize-none placeholder:text-slate-400"
                    />
                  </div>
                )}
        ...
        {docToDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            ...
            >
              ...
              <h3 className="font-bold text-slate-800 text-base mb-1">Delete Document?</h3>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold text-xs transition-all cursor-pointer"
                >
                  Cancel
                </button>
                <button
